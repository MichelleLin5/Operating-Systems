...
netid: ml7188
CITATIONS:
https://www.cnblogs.com/smartjourneys/p/8430930.html
...
int index_in_permutations_list;
...
void option_l (char * disk) 
{
    ...
    DirEntry * directory_entry = (DirEntry *)(disk + boot_entry->BPB_BytsPerSec * (boot_entry->BPB_RsvdSecCnt + boot_entry->BPB_FATSz32 * boot_entry->BPB_NumFATs + boot_entry->BPB_SecPerClus*(boot_entry->BPB_RootClus-2)));
    ...
    while (next_cluster < 0x0ffffff8)
    {
        ...
        entry_per_cluster = boot_entry->BPB_BytsPerSec * boot_entry->BPB_SecPerClus/32;
        ...
        if (root_cluster_count >= 1)
        {
            ...
            else //when root_cluster_count > 1
            {
        ...
        directory_entry = (DirEntry *)(disk + boot_entry->BPB_BytsPerSec * (boot_entry->BPB_RsvdSecCnt + boot_entry->BPB_FATSz32 * boot_entry->BPB_NumFATs + boot_entry->BPB_SecPerClus*(next_cluster-2)));
    }
...
void option_r (int fd, BootEntry * disk, char * file_name) //recover contiguous file when no sha1 hash is given
{
    ...
    while (1)
    {
        ...
        for(int i = 0; i < (int)possible_entries; i++)
        {
    ...
    if (file_count == 1) //only one match, proceed to recover file
    {
        ...
        else if (number_of_clusters == 1) //non-empty file with only one cluster
        {
            ...
            FAT[starting_cluster] = 0xffffffff;
        ...
        else //non-empty file with contiguous cluster(s)
        {
            ...
            {
                for (int j = starting_cluster; j < starting_cluster + number_of_clusters - 1; j++)
                {
                    FAT[j + i * (fat_size)] = j + 1;
                }
                FAT[starting_cluster + number_of_clusters - 1] = 0x0fffffff;
            }
...
{
    unsigned int current_cluster = disk->BPB_RootClus;
    unsigned int possible_entries_per_cluster = (disk->BPB_SecPerClus * disk->BPB_BytsPerSec)/sizeof(DirEntry);
    ...
    while (1)
    {
        ...
        unsigned int current_cluster_offset = ((disk->BPB_RsvdSecCnt + disk->BPB_NumFATs * disk->BPB_FATSz32) + (current_cluster - 2) * disk->BPB_SecPerClus) * disk->BPB_BytsPerSec;
        directory_entry = (DirEntry*)(file_content + current_cluster_offset);

        for(int i = 0; i < (int)possible_entries_per_cluster; i++)
        {
            if (directory_entry->DIR_Attr == 0x00)
            {        
                break;
            ...
            {  
                if (directory_entry->DIR_Name[1] == file_name[1])
                {
                    ...
                    if(strcmp(recorded_filename+1,file_name+1)==0)
                    {
                        ...
                        {
                            unsigned char * content = malloc(sizeof(char) * (directory_entry->DIR_FileSize + 1)); //file content
                            ...
                            if (((directory_entry->DIR_FileSize) % (disk->BPB_SecPerClus * disk->BPB_BytsPerSec)) != 0) //如果不能整除就得+1
                            {
                                number_of_clusters++;
                            }
    ...
    else if (directory_entry_temp != NULL)  //found
    {
        ...
        else if (number_of_clusters == 1) //non-empty file with only one cluster
        {
            ...
            FAT[starting_cluster] = 0xffffffff; //starting cluster is EOF
        ...
        else //non-empty file with contiguous cluster(s)
        {
            ...
            int starting_cluster = (directory_entry_temp->DIR_FstClusHI * 65536 + directory_entry_temp->DIR_FstClusLO); //find the first cluster of the file to recover 
            unsigned int fat_size = (disk->BPB_FATSz32 * disk->BPB_BytsPerSec);
            ...
            for (int i = 0; i < disk->BPB_NumFATs; i++)
            {
                ...
                FAT[starting_cluster + number_of_clusters - 1] = 0xffffffff;
            }
...
{
    unsigned int current_cluster = disk->BPB_RootClus;
    unsigned int possible_entries_in_each_cluster = (disk->BPB_SecPerClus * disk->BPB_BytsPerSec)/sizeof(DirEntry);
    ...
    while (1)
    {
        ...
        for(int i = 0; i < (int)possible_entries_in_each_cluster; i++)
        {
            if (directory_entry->DIR_Attr == 0x00)
            {        
                break;
                ...
                if (directory_entry->DIR_Name[1] == file_name[1])
                {
                    ...
                    if(strcmp(recorded_filename+1,file_name+1)==0)
                    {
                        ...
                        else 
                        {
                            ...
                            {
                                if (permutations_list[perms][0] == (int)fileStartClu)
                                {
                                    //put the file content in
                                    int iterator = 0;
                                    for (int i = 0; i < number_of_clusters; i++)
    ...
    else if (directory_entry_temp != NULL)  //found
    {
        ...
        else if (number_of_clusters == 1) //non-empty file with only one cluster
        {
            ...
            FAT[starting_cluster + 1] = 0xffffffff;
        ...
        else //non-empty file with non-contiguous cluster(s)
        {
            ...
            for (int i = 0; i < disk->BPB_NumFATs; i++)
            {
                for (int j = 0; j < number_of_clusters; j++)
                {
                    unsigned int cluster_recovering = permutations_list[index_in_permutations_list][j];
                    // fprintf(stderr, "recovering cluster %d\n", cluster_recovering);
                    if (j == number_of_clusters - 1)
                    {
                        FAT[cluster_recovering + (i * (fat_size))] = cluster_recovering + 1;
                    }
                    else
                    {
                ...
                FAT[(permutations_list[index_in_permutations_list][number_of_clusters - 1] + i * (fat_size)) + 1] = 0x0fffffff;
            }
